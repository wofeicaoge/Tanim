...
from tanim.utils.sounds import play_error_sound
from tanim.utils.sounds import play_finish_sound
from tanim.utils.constants import CHOOSE_NUMBER_MESSAGE, INVALID_NUMBER_MESSAGE, NO_SCENE_MESSAGE, SCENE_NOT_FOUND_MESSAGE
...
def prompt_user_for_choice(scene_classes):
    ...
    try:
        ...
        sys.exit(2)
        user_input = input(tanim.utils.constants.CHOOSE_NUMBER_MESSAGE)
        return [
            num_to_class[int(num_str)]
            for num_str in user_input.split(",")
        ]
...
def get_scenes_to_render(scene_classes, config):
    ...
    for scene_name in config["scene_names"]:
        ...
        if not found and (scene_name != ""):
            print(
                SCENE_NOT_FOUND_MESSAGE.format(
                    scene_name
                ),
                file=sys.stderr
            )
